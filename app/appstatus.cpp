...
AppStatus::AppStatus(AccountController *account, SyncController *sync, Clock clock, QObject *parent)
    : QObject(parent)
    , m_account(account)
    , m_sync(sync)
    ...
    , m_tick(new QTimer(this))
{
    ...
    connect(m_tick, &QTimer::timeout, this, &AppStatus::tick);
    connect(m_account, &AccountController::serviceAvailableChanged, this, &AppStatus::update);
    connect(m_account, &AccountController::accountChanged, this, &AppStatus::update);
    connect(m_sync, &SyncController::serviceAvailableChanged, this, &AppStatus::update);
    connect(m_sync, &SyncController::syncChanged, this, &AppStatus::update);
    connect(m_sync->transfers(), &TransferModel::countChanged, this, &AppStatus::update);
    update();
}

QString AppStatus::iconName() const
{
    if (m_state == QLatin1String("ok")) {
        return QStringLiteral("state-ok");
    }
    if (m_state == QLatin1String("syncing")) {
        return QStringLiteral("state-sync");
    }
    if (m_state == QLatin1String("warning")) {
        return QStringLiteral("state-warning");
    }
    return QStringLiteral("state-offline");
}

QString AppStatus::ago(qint64 unixSeconds) const
{
...
void AppStatus::update()
{
    ...
    if (!m_account->serviceAvailable() || !m_sync->serviceAvailable()) {
        ...
        const bool helperTrouble = m_sync->helperTrouble();
